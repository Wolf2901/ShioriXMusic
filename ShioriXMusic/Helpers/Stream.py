...
from ShioriXMusic import BOT_NAME, BOT_USERNAME, SUDO_USERS, db_mem
from ShioriXMusic.Helpers.PyTgCalls import Queues, ShioriX
from ShioriXMusic.Helpers.Database import (add_active_chat, is_active_chat, music_off,
                            music_on)
from ShioriXMusic.Helpers.Inline import audio_markup, primary_markup
...
async def start_stream(
    ...
    if await is_active_chat(message.chat.id):
        ...
        if file not in db_mem:
            db_mem[file] = {}
        cpl = f"ShioriXMusic/Cache/{_path_}.png"
        ...
        try:
            await ShioriX.pytgcalls.join_group_call(
                message.chat.id,
                InputStream(
                    InputAudioStream(
                        file,
...
async def start_stream_audio(
    ...
    if await is_active_chat(message.chat.id):
        ...
        got_queue.append(to_append)
        final_output = await message.reply_photo(
            photo="ShioriXMusic/Utilities/Audio.jpeg",
            caption=(
