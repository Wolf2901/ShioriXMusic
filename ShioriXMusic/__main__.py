...
from ShioriXMusic.Helpers.PyTgCalls.ShioriX import run
from ShioriXMusic.Modules import ALL_MODULES
from ShioriXMusic.Helpers.Inline import private_panel
from ShioriXMusic.Helpers.Database import get_active_chats, remove_active_chat, add_served_user
from ShioriXMusic import (ASSID, ASSMENTION, ASSNAME, ASSUSERNAME, BOT_ID, BOT_NAME, BOT_USERNAME, SUDO_USERS, F_OWNER, db, app, Ass)
...
async def ShioriX_boot():
    with console.status(
        ...
        for all_module in ALL_MODULES:
            imported_module = importlib.import_module(
                "ShioriXMusic.Modules." + all_module
            )
            if (
    ...
    try:
        ...
        return
    try:
        await Ass.join_chat("Silent_robo_11")
        await Ass.join_chat("Mine_Bots")
    except:
        pass
 ...
 sᴇᴀʀᴄʜ ᴩᴏᴡᴇʀᴇᴅ ʙʏ {BOT_NAME} 🥀"""
    ...
    return await message.reply_photo(
        ...
        [
            ...
            [
                InlineKeyboardButton(
                    text="❄ ʜᴇʟᴩ ❄", callback_data="ShioriX_help"
                ),
                InlineKeyboardButton(
            ...
            [
                InlineKeyboardButton(
                    text="☁ sᴏᴜʀᴄᴇ ᴄᴏᴅᴇ ☁", url="https://github.com/Wolf2901/ShioriXMusic"
                )
            ],
        ]
    ),
...
@app.on_callback_query(filters.regex("ShioriX_home"))
async def ShioriX_home(_, CallbackQuery):
    await CallbackQuery.answer("Sʜɪᴏʀɪ X ʜᴏᴍᴇ")
